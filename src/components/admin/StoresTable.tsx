...
export default function StoresTable() {
  ...
  const [selectedStoreForOrder, setSelectedStoreForOrder] = useState<Store | null>(null);
  ...
  const handleUpdateStoreOrder = async () => {
    ...
    try {
      const updatedStore = await updateStoreOrder(selectedStoreForOrder._id, orderData);
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
            ...
            <div className="p-4 space-y-4">
              <div>
                <h4 className="text-sm font-medium text-gray-500">Bank Details</h4>
                <div className="mt-2 space-y-2">
                  <p className="text-sm text-gray-900">
                    <span className="font-medium">Account Name:</span> {selectedStore.paymentDetails.accountName}
                  </p>
                  <p className="text-sm text-gray-900">
                    <span className="font-medium">Account Number:</span> {selectedStore.paymentDetails.accountNumber}
                  </p>
                  <p className="text-sm text-gray-900">
                    <span className="font-medium">Bank Name:</span> {selectedStore.paymentDetails.bankName}
                  </p>
                </div>
              </div>
              <div>
                ...
                <div className="mt-2 space-y-2">
                  <p className="text-sm text-gray-900">
                    <span className="font-medium">Email:</span> {selectedStore.contactInfo.email}
                  </p>
                  <p className="text-sm text-gray-900">
                    <span className="font-medium">Phone:</span> {selectedStore.contactInfo.phone}
                  </p>
                  {selectedStore.contactInfo.whatsapp && (
                    <p className="text-sm text-gray-900">
                      <span className="font-medium">WhatsApp:</span> {selectedStore.contactInfo.whatsapp}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-200 rounded-lg p-4 border border-gray-300 shadow-sm">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setOrderData({...orderData, displayOrder: value ? parseInt(value) : 0});
                    }}
                ...
                <div className="bg-gray-200 rounded-lg p-4 border border-gray-300 shadow-sm">
                  ...
                  <div className="flex items-center mb-4">
                    <input
                      ...
                      onChange={(e) => setOrderData({...orderData, isFeatured: e.target.checked})}
                  ...
                  {orderData.isFeatured && (
                    <div>
                      ...
                      <input
                        type="datetime-local"
                        value={orderData.featuredUntil ? new Date(orderData.featuredUntil).toISOString().slice(0, 16) : ''}
                        onChange={(e) => setOrderData({...orderData, featuredUntil: e.target.value})}
                        className="block w-full px-3 py-2 bg-white border border-gray-400 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900"
                      />
                ...
                <div className="bg-gray-200 rounded-lg p-4 border border-gray-300 shadow-sm">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setOrderData({...orderData, adminNotes: e.target.value})}
                    rows={3}
                    className="block w-full px-3 py-2 bg-white border border-gray-400 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900"
